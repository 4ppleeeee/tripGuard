...
internal class KmmPopManager(private val context: IKmmContext) : IPopUpManager {
    ...
    private fun checkShow(popTask: KmmPopTask?): PopResult {
        if (popTask == null) {
        ...
        val result = synchronized(lock) {
            when {
                ...
                else -> checkPriorityConflict(popTask)
            }
        }
        return result
    }
    ...
    /** 检查优先级冲突 - 必须在锁内调用 */
    private fun checkPriorityConflict(popTask: KmmPopTask): PopResult {
        // 检查同ID弹窗
        if (!popTask.canSameIdDialogShow) {
            val sameIdTask = showingDialogList.find { it.id == popTask.id }
            if (sameIdTask != null) {
                logger.fileLog(
                    "SameId",
        ...
        // 检查高优先级冲突
        val higherTask = showingDialogList.find { popTask < it && popTask.isSamePosition(it) }
        if (higherTask != null) {
            logger.fileLog(
                "Priority",
    ...
    override fun removeItem(popTask: KmmPopTask?): Boolean {
        val result = synchronized(lock) {
            if (showingDialogList.contains(popTask)) {
                ...
                if (removed) {
                    popTask?.lifecycleObserver?.onDismiss(popTask)
                }
    ...
    companion object {
        ...
        private val TALKBACK_INTERCEPT_TYPES = setOf(
            ...
            PopType.OLYMPIC_PENDANT,
            PopType.HIGHLIGHT_PENDANT,
            PopType.AD_FOLLOW_U,
