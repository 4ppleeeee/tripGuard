...
open class NewsListWidget() : StructWidget(), IFeedsItemWidget {
    ...
    fun canTopRefresh(): Boolean {
        return pickTopRefreshRequest() != null
    }
    ...
    fun pickTopRefreshRequest(): DataRequest? {
        return action?.top_refresh?.pickAnyRequest()
    }
    ...
    // 顶部下拉刷新（替换整个列表）
    fun buildTopRefreshAction(request: DataRequest) {
        nonNullAction.top_refresh = DataRequestAction.createAutoRequest(request)
    }
...
class NewsListWidgetAction : StructWidgetAction() {
    ...
    var top_refresh: DataRequestAction? = null  // 顶部刷新
