...
data class WXLoginInitResult(
    val appId: String,
)

class WXLoginInitTask(
    private val initWXLogin: PlatformTask<WXLoginInitResult>
) : StartupTask {
    ...
    override suspend fun execute(context: StartupContext) {
        initWXLogin(context) { _ ->
            // 预留扩展：后续接入登录态缓存或埋点时在此处理结果
        }
    }

    companion object {
        const val TASK_ID = "wx-login"
    }
}
