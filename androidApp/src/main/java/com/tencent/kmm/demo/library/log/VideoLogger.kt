...
/**
 * Android 视频日志文件 Logger。
 *
 * 设计要点：
 * - 目录：`{WsLogger 主日志目录的父目录}/video`
 * - 文件名：`video_yyyyMMdd.log`，按天滚动；单文件上限 10MB，超过则切出 `video_yyyyMMdd_HHmmss.log`
 * - 线程模型：单线程异步写入（阻塞队列），避免拖慢业务线程
 * - 保留策略：最多保留最近 [MAX_RETAIN_DAYS] 天的日志文件，启动时和每天首次写入时清理
 * - 级别过滤：debug 包 d/i/w/e 全量；release 包仅 w/e
 * - 编码：UTF-8 明文（视频日志不含敏感信息）
 * - flush：同步等待队列清空后 fsync 到磁盘
 */
object WsVideoLogger {

    private const val TAG_SELF = "WsVideoLogger"
    private const val SUB_DIR = "video"
    ...
    private var isDebug: Boolean = true // appStatus().isDebug()
    ...
    private val executor = Executors.newSingleThreadExecutor(object : ThreadFactory {
        override fun newThread(r: Runnable): Thread = Thread(r, "WsVideoLogger-Writer").apply {
            isDaemon = true
    ...
    fun setup(context: Any?, isDebug: Boolean) {
        ...
        try {
            val dir = resolveVideoLogDir(ctx)
            if (dir == null) {
            ...
            logDir = dir
            Log.i(TAG_SELF, "setup: videoLogDir=${dir.absolutePath}, isDebug=$isDebug")
            executor.execute {
                cleanExpired()
                ensureWriter()
            }
    ...
    /**
     * 视频日志目录 = 主日志目录的**父目录** 下的 `video` 子目录。
     * 若主日志目录未就绪，则回退到 `{files}/logs_parent/video`（仅兜底，正常不会走到）。
     */
    private fun resolveVideoLogDir(ctx: Context): File? {
    ...
    private fun ensureWriter(): OutputStreamWriter? {
        ...
        val file = File(dir, "video_$now.log")
        currentFile = file
        return try {
    ...
    private fun rolloverBySize() {
        ...
        val f = currentFile
        if (f != null && f.exists()) {
            runCatching { f.renameTo(File(dir, "video_${stamp}.log")) }
        }
        ensureWriter()
    }
    ...
    private fun cleanExpired() {
        ...
        runCatching {
            dir.listFiles()?.forEach { f ->
                if (f.isFile && f.name.startsWith("video_") && f.lastModified() < keepAfter) {
                    f.delete()
                }
            }
        }
