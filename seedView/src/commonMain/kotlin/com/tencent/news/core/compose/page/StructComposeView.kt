...
import com.tencent.news.core.compose.scaffold.TitleHangingViewImpl
...
internal fun StructComposeView(
    ...
    // 触发更新，不能删
    pageWidget.pager?.asWidgetVM?.structureVersionFlow?.collectAsState()?.value
    ...
    CompositionLocalProvider(
        ...
        StructPageScaffold(
            ...
            enableRootPullRefresh = uiCustomize?.enableRootPullRefresh ?: false,
            ...
            titleHangingView = pageWidget.structTitleHangingView(),
            channelBar = pageWidget.structChannelBar(
                pagerState,
                uiCustomize?.onChannelSwitchIntercept,
            ),
...
private fun StructPageWidget2.structTitleHangingView(): TitleHangingViewImpl? =
    titleHanging?.let { widget ->
        {
            debugLog { "TitleHanging 发生重组，请确认是否应该优化！" }
            ViewService.hanging.Build(widget)
        }
    }

private fun StructPageWidget2.structChannelBar(
    ...
    pager?.channelBar?.takeIf { it.canShowChannelBar() }?.let { widget ->
        {
            ...
            ViewService.channelBar.Build(pagerState, widget, onChannelSwitchIntercept)
        }
    }
            ...
            Box(modifier = containerModifier) {
                if (pageWidget.hasMultiChannels()) {
                    StructMultiTabContent(
                        ...
                        uiCustomize?.onChannelSwitchIntercept,
                        uiCustomize?.disableChannelSwipe == true,
                    )
                } else {
...
private fun getPagerState(pagerWidget: PagerWidget?): PagerState {
    // 优先使用 channelBarVM 中保存的 selectedIndex 恢复 pager 位置
    // 当 tab 切走再切回时，pagerState 会被重新创建，但 channelBarVM 的 selectedIndex 仍保留之前的值
    val restoredIndex = pagerWidget?.channelBar?.vm?.selectedIndex?.value
        ?: pagerWidget?.action?.initIndex ?: 0
