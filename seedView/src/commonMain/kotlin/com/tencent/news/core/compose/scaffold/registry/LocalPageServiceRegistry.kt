...
val LocalStructVerticalPagerIndex = service<State<Int>> { mutableIntStateOf(0) }

// 【品字形框架】VerticalPager 当前页偏移比例（0f = 完全停稳，非 0 = 正在滑动）
val LocalStructVerticalPagerOffsetFraction = service<State<Float>> { mutableStateOf(0f) }

// 【品字形框架】VerticalPager 滑动半透明效果的发起页索引（-1 = 当前没有需要半透明的页）
val LocalStructVerticalPagerSwipeAlphaSourceIndex = service<State<Int>> { mutableIntStateOf(-1) }
...
// 注意：开启 contentFullScreen 模式后，这个是0
val LocalStructChannelOffset = service<State<Dp>> { mutableStateOf(0.dp) }

// 【品字形框架】顶部悬停空间（一般是：TitleBar+ChannelBar+悬停区高度）
// 注意：这个不受 contentFullScreen 影响，一直是恒定值
val LocalStructTitleAreaHeight = service<State<Dp>> { mutableStateOf(0.dp) }
...
// 【页面通用】监听安全区刷新事件
val LocalComposeSafeAreaInsetsFlow = service<SharedFlow<Map<String, Any>>?> { null }
...
// 【页面通用】toast能力，复用最顶级的DialogController
val LocalToastController = service { DialogController() }

// 【页面通用】全屏能力（视频全屏播放等场景）
val LocalFullScreenController = service { FullScreenController() }
...
@KmmInternalApi // 品字形内部使用，主要给 TitleHanging
val LocalHeaderCollapseStatusInner = service<State<Boolean>> { mutableStateOf(false) }

// 【品字形框架】自定义错误页面图片Provider
val LocalErrorImagePainterProvider = service<(@Composable () -> Painter)?> { null }

// 【品字形框架】嵌套滚动配置（同时支持 HorizontalPager 和纵向列表）
val LocalNestedScrollConfig = service<NestedScrollConfig?> { null }