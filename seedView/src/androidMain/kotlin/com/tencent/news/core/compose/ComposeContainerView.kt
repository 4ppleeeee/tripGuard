...
class ComposeContainerView @JvmOverloads constructor(
    ...
    private val lifecycleObserver = LifecycleEventObserver { source, event ->
        if (event == Lifecycle.Event.ON_DESTROY) {
            ...
            // https://bugly.woa.com/v2/memory/java-leak/detail?productId=0d8bed2efe&pid=1&token=55e814e99ce59fa6f418a0ace00cef46&feature=0b24b0e587a788d47b0c40035a3e8d62&cId=a0f8f0470a71e4efc451708712be02ce
            innerDestroy()
        }
    }
    ...
    override fun onCreate(composeViewKey: String, pageArgs: IComposePageArgs) {
        ...
        // view复用时，要么彻底回收旧的compose；要么改为发送自定义事件，让compose侧自己处理刷新
        if (compose != null) {
    ...
    private fun realCreateComposeView() {
        ...
        val compose = AndroidComposePageDelegate.create()
    ...
    private fun IComposePageArgs.getViewSize(): Size? {
        ...
        val heightPx = initHeightInDp.dpToPx()
    ...
    override fun onAttachedToWindow() {
        ...
        if (autoDestroyOnDetach &&
            ...
            compose == null // 有数据但compose为空，可能是生命周期不配对：例如detach回收后，没有重新走onCreate
        ) {
