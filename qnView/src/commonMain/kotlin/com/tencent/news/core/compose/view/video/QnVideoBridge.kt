...
class QnVideoBridge(
    ...
    override fun onNetVideoInfo(info: QnNetVideoInfo) {
        super.onNetVideoInfo(info)
        QnVideoLog.log(
            "onNetVideoInfo payState=${info.mediaVideoState}, st=${info.st}, " +
                "previewDurationSec=${info.previewDurationSec}, isPreview=${info.isPreview}"
        )
        dispatchLifeCycle(QnVideoPlayState.ON_NET_VIDEO_INFO) {
    ...
    override fun onPermissionTimeout() {
        super.onPermissionTimeout()
        QnVideoLog.log("onPermissionTimeout")
        dispatchLifeCycle(QnVideoPlayState.ON_PERMISSION_TIMEOUT)
    }
    ...
    private fun handleCallMethod(
        ...
        when (method) {
            ...
            QnVideoControllerInvoker.SEEK_TO -> {
                val positionSeconds = (params as? String)?.toLongOrNull() ?: 0L
                qnVideo?.getVideoController()?.seekTo(positionSeconds)
            }
            QnVideoControllerInvoker.SET_SPEED -> {
                val speed = (params as? String)?.toFloatOrNull() ?: 1.0f
                qnVideo?.getVideoController()?.setSpeed(speed)
            }
