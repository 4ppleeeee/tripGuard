...
class QnVideoControllerComposeInvoker(
    ...
    var seekToAction: ((positionSeconds: Long) -> Unit)? = null
    var setSpeedAction: ((speed: Float) -> Unit)? = null
    ...
    override fun seekTo(positionSeconds: Long) {
        log("seekTo positionSeconds:$positionSeconds")
        seekToAction?.invoke(positionSeconds)
    }

    override fun setSpeed(speed: Float) {
        log("setSpeed speed:$speed")
        setSpeedAction?.invoke(speed)
    }
}
...
internal class QnVideoControllerNativeInvoker(
    ...
    override fun seekTo(positionSeconds: Long) {
        log("seekTo positionSeconds:$positionSeconds")
        qnVideoView.callRenderViewMethod(
            methodName = QnVideoControllerInvoker.SEEK_TO,
    ...
    override fun setSpeed(speed: Float) {
        log("setSpeed speed:$speed")
        qnVideoView.callRenderViewMethod(
            methodName = QnVideoControllerInvoker.SET_SPEED,
            speed.toString()
        )
    }
    ...
    override fun bindAction(state: QnVideoState) {
        ...
        invoker.seekToAction = {
            seekTo(it)
        }
        invoker.setSpeedAction = {
            setSpeed(it)
        }
    }
}