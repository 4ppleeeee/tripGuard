...
fun TextField(
    ...
    requestFocusOnce: Boolean = false,
    ...
    val managedFocusRequester = remember { FocusRequester() }
    val appliedFocusRequester = focusRequester ?: managedFocusRequester
    ...
    val updatedModifier = modifier
        ...
        .focusRequester(appliedFocusRequester)
        .onGloballyPositioned {
            isPositioned = true
        }
        .onFocusChanged {
    ...
    LaunchedEffect(autoFocus, requestFocusOnce, isPositioned) {
        if ((autoFocus || requestFocusOnce) && isPositioned) {
            delay(100)
            runCatching {
                appliedFocusRequester.requestFocus()
            }
        }
    }
}
...
/**
 * 卡片中心点 Y 坐标相对"整页可视高度"（pageViewHeightValue）的比例。
 *
 * 语义：
 * - 0f = 中心在页面顶部，1f = 中心在页面底部；
 * - 中心滑出可视区时返回可能 < 0 或 > 1（保留溢出量，便于判定"已离屏"）；
 * - 与 [centerYPercentage] 的区别：此函数以"视觉屏幕/整页"为参照，
 *   不受直接父容器高度、祖先布局变化的影响，适合"中心带自动播放"类需求。
 *
 * ⚠️ 单位对齐：
 * - [layoutCoordinates.boundsInRoot] 返回的是像素（px）坐标；
 * - [pageViewHeightValue] 返回的是 dp 数值；
 * - 必须经 [LocalDensity] 把 px → dp 再做除法，否则比例会被放大 density 倍，
 *   在 3x 设备上 percent 直接被 [coerceIn] 夹到 1.0，导致中心带判定永远不成立。
 *
 * 当页面高度无效（未测量完成）时不会回调。
 *
 * 回调参数：
 * - percent：中心 Y 在整页高度中的比例（可能 < 0 或 > 1，未夹紧）
 * - centerYDp：中心 Y 的 dp 值
 * - pageHeightDp：整页高度的 dp 值
 */
fun Modifier.centerYInPagePercentage(
    onPercentageChanged: (percent: Float, centerYDp: Float, pageHeightDp: Float) -> Unit
): Modifier = composed {
    val pageHeightDp = pageViewHeightValue()
    val density = LocalDensity.current
    this.onGloballyPositioned { layoutCoordinates ->
        if (pageHeightDp <= 0f) return@onGloballyPositioned
        val bounds = layoutCoordinates.boundsInRoot()
        val centerYPx = (bounds.top + bounds.bottom) / 2f
        val centerYDp = with(density) { centerYPx.toDp().value }
        val percent = centerYDp / pageHeightDp
        onPercentageChanged(percent, centerYDp, pageHeightDp)
    }
}
