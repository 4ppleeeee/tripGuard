...
private const val USER_INTERACTION_ENABLED = "user_interaction_enabled" // 是否允许用户交互
private const val ENABLE_REUSE = "enable_reuse" // 是否启用原生复用池
private const val REUSE_KEY = "reuse_key" // 原生复用池key
...
fun QnWebViewCell(
    ...
    state: WebState,
    url: String,
    userInteractionEnabled: Boolean = true, // 默认允许交互
    enableReuse: Boolean = true,
    ...
    MakeKuiklyComposeNode<QnWebViewCell>(
        ...
        viewUpdate = {
            it.getViewAttr().run {
                ...
                with(STATE, state)
                with(THEME, isDark)
                with(USER_INTERACTION_ENABLED, userInteractionEnabled)
                with(ENABLE_REUSE, enableReuse)
                if (!reuseKey.isNullOrEmpty()) {
                    with(REUSE_KEY, reuseKey)
                }
            }
        }
    )
