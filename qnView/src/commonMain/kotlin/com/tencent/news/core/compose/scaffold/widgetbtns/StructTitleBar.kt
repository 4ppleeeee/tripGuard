...
fun StructTitleBar(
    ...
    val titleBarTheme = provideDefaultTitleBarTheme(
        ...
        isIconAlwaysDark = isIconAlwaysDark,
        isAlwaysShowTitle = isAlwaysShowTitle,
        forceLightTitleInTransparentBg = widget.ui.forceLightTitleInTransparentBg
    )
...
fun ThemedStructTitleBar(
    ...
    val skinBarBg = QnSkin?.barBgColor
    val themeBg = currentTitleBarTheme.titleBarBgColor
    // 换肤 titleBar 的背景图绘制在 StructTitleBar 下方，透明配置必须真正透出底层图片。
    val bgColor = when {
        widget.ui.alwaysTransparentBg -> QnColor.transparent
        isHeaderCollapsed != null && skinBarBg != null -> {
...
private fun RowScope.LeftBtns(widget: CommonTitleBarWidget) {
    Row(
        ...
        if (!widget.ui.hideBackBtn) {
            BackBtn(iconSize = widget.ui.backBtnIconSize, fontWeight = widget.ui.backBtnFontWeight)
        }

        if (widget.ui.alwaysShowLeftBtns ||
            currentTitleBarTheme.isHeaderCollapsed && !currentTitleBarTheme.hideLeftBtns
        ) {
