...
/**
 * 固定视觉尺寸的dp单位：用于图片、容器高度、固定间距等不应跟随字号缩放的场景。
 */
@Composable
@Stable
expect fun fixedDp(value: Float): Dp
...
fun IOS375Scale(content: @Composable () -> Unit) {
    if (isIOSPlatform()) {
        val scaleBy375 = appStatus().getScreenWidth().toFloat() / 375F
...
@Composable
fun safeAreaHeight(): Float {
    if (isAndroidPlatform()) {
        return 0f
    }
    return LocalConfiguration.current.safeAreaInsets.bottom
}
