...
fun QnLottie(
    ...
    val tintColorValue = remember(tintColor) {
        tintColor?.toKuiklyColor()?.hexColor?.toString(16) ?: ""
    }
    val data = remember(
        fileName,
        status,
    ...
    // DATA 是静态配置，progress 单独下发；避免进度重组时重复序列化整份 Lottie 配置。
    val dataProp = remember(data) {
        data.toProp()
    }
