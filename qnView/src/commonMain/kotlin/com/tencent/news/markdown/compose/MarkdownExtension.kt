...
import com.tencent.news.markdown.model.MarkdownCustomElementTypes.VIDEO
import com.tencent.news.markdown.model.MarkdownState
import com.tencent.news.markdown.model.ParseResult
import com.tencent.news.markdown.model.findVideoPlaceholderIndexOrNull
...
internal const val VIDEO_INDEX_EXTRA_KEY = "video_index"
...
internal fun handleElement(
    ...
    when (node.type) {
        ...
        VIDEO -> {
            val videoModel = model.copy(
                extra = persistentMapOf(VIDEO_INDEX_EXTRA_KEY to (node.findVideoPlaceholderIndexOrNull(content()) ?: -1))
            )
            components.video(videoModel)
        }
        LINK_DEFINITION -> {
