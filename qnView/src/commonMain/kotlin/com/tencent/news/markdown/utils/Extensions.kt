package com.tencent.news.core.compose.view.markdown.utils
...
import com.tencent.news.markdown.utils.EntityConverter
...
/**
 * Tag used to indicate a ref link for inline content. Required for rendering.
 */
const val MARKDOWN_TAG_REF_URL = "MARKDOWN_REF_URL"
...
fun String.decodeMarkdownHtmlEntities(): String {
    return EntityConverter.replaceEntities(
        this,
 ...
 */
internal fun List<ASTNode>.mapAutoLinkToType(targetType: IElementType = MarkdownTokenTypes.TEXT): List<ASTNode> {
    return map {
...
internal fun lookupLinkDefinition(
    ...
    val linkLabel = if (node.type == MarkdownElementTypes.LINK_DEFINITION) {
        ...
        linkOnly = true
        (node.children.firstOrNull { it.type.name == MarkdownElementTypes.AUTOLINK.name }
...
internal val MarkdownTypography.codeSpanStyle: SpanStyle
    @Composable
    get() =
        if (LocalMarkdownColors.current.inlineCodeText != Color.Unspecified) {
            inlineCode.merge(
