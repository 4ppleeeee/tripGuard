...
object NetworkRelayService : SynchronizedObject() {
    ...
    // 不参与cacheKey匹配的顶层body参数集合，这些参数在预加载和真实请求之间可能变化，忽略后可保证接力生效。
    private val blackBizParamKeys by lazy {
        val def = listOf(
            "isCpFocus",            // CP焦点状态，预加载时为0，真实请求时可能为1
        )
        getShiplyStringList("network_relay_black_biz_list", def) ?: emptyList()
    }
    ...
    private fun <T> sendRequest(
        ...
        if (networkProxy != null && networkProxy.response.isExpired()) {
            ...
            NetworkLog.fileLog("${id}接力请求已过期：$cacheKey，重新发起新请求")
            networkProxy = null
        }
        ...
        if (networkProxy != null && networkProxy.response.isFailed()) {
            ...
            NetworkLog.fileLog("${id}接力请求失败：$cacheKey，重新发起新请求")
            networkProxy = null
        }
        ...
        if (networkProxy == null) {
            ...
            // 2. 发起网络请求
            NetworkLog.fileLog("${id}接力尚未开始，替换onResponse为${responseProxy}并发起网络请求：$cacheKey")
            ...
            // 接力已开始，且肯定未过期
            NetworkLog.fileLog("${id}接力已开始，等待网络响应：$cacheKey")
    ...
    private fun Map<String, Any>?.flatToStableString(): String? {
        return this?.entries
            ?.filter { !blackBizParamKeys.contains(it.key) }
            ?.sortedBy { it.key }
            ?.joinToString {
                if (it.key == "adReqData") {
...
private class NetworkRelayResponseProxy<T>(
    ...
    private fun invokeOnSuccess(netResponse: INetworkResponse<T>) {
        ...
        NetworkLog.debug { "${leadOffReceiver.id}接力结束，网络请求回来了: ${cacheKey}, 共${otherReceivers.size}个接力选手" }
        // 通知第一棒，网络请求回来了
    ...
    fun addRelayReceiver(receiver: NetworkBuilder<T>) = synchronized(NetworkRelayService) {
        ...
        if (netResponse != null && this.isNetRequestFinished) {
            ...
            NetworkLog.debug { "添加接力选手${receiver.id}时，${leadOffReceiver.id}网络请求已经回来了，直接回调：$cacheKey" }
            receiver.onResponse.invoke(response)
        } else {
            NetworkLog.debug { "添加接力选手${receiver.id}，等待第一棒的网络响应：$cacheKey" }
